...
public class PublishView extends ParameterizedViewImpl implements PublishPresenter.MyView {
	...
	public void initializeParameters(String[] params) {
		...
		if (session == null) {
			...
			// this.api.notifyFailure("You need to be logged in to edit software
			// description");
			return;
		}
	...
	private void initTable() {
		...
		TextColumn<Authorization> isownercolumn = new TextColumn<Authorization>() {
			@Override
			public String getValue(Authorization auth) {
				return "";
			}
	...
	private void initAgents() {
		ArrayList<Authorization> authorizations = new ArrayList<Authorization>(
				this.software.getPermission().getAuthorizations().values());
		...
		for (Iterator<Authorization> iter = authorizations.listIterator(); iter.hasNext();) {
			...
			if (!permusers.contains(auth.getAgentName()) && auth.getAccessMode().getMode().equals("Write")
					&& auth.getAccessToObjId().equals(this.software.getId()) && !auth.getAgentName().equals("*")) {
				authlist.add(auth);
	...
	private void setPermButtonVisibility() {
		this.api.getPermissionFeatureEnabled(new Callback<Boolean, Throwable>() {
			...
			public void onSuccess(Boolean permEnabled) {
				...
				if (permEnabled && ((session != null && session.getRoles().contains("admin"))
		...
		this.api.getSoftware(softwarename, new Callback<Software, Throwable>() {
			...
			public void onSuccess(Software sw) {
				...
				permissiondialog.setTitle(
						"Set Permissions for " + swlabel.substring(0, 1).toUpperCase() + swlabel.substring(1));
			}
	...
	private void initialDraw() {
		...
		if (piechart.getActiveCategoryId() != null) {
			MetadataCategory mcat = piechart.getVocabulary().getCategory(piechart.getActiveCategoryId());
			if (mcat != null) {
		...
		if (barchart.getActiveCategoryId() != null) {
			MetadataCategory mcat = barchart.getVocabulary().getCategory(barchart.getActiveCategoryId());
			if (mcat != null) {
	...
	void onBarSelect(CategorySelectionEvent event) {
		...
		if (bcat != null && pcat != null) {
			History.replaceItem(publishPlace + "/" + softwarename + "/" + pcat.getName() + "/" + bcat.getName(), false);
		...
		UserREST.getUserRoles(username, new Callback<List<String>, Throwable>() {
			...
			public void onSuccess(List<String> roles) {
				if (roles != null && roles.contains("admin")) {
					...
					api.getSoftwareAccessLevelForUser(software.getName(), username,
	...
	private void submitPermissionForm() {
		...
		if ((session != null && session.getRoles().contains("admin"))
			...
			if (ownerrole.getValue() == true) {
				this.api.addSoftwareOwner(software.getName(), username, new Callback<Boolean, Throwable>() {
					@Override
					public void onFailure(Throwable reason) {
