...
public class SoftwareForm extends Composite implements HasSoftwareHandlers, HasPluginHandlers {
	...
	@Override
	public HandlerRegistration addSoftwareSaveHandler(SoftwareSaveHandler handler) {
		return handlerManager.addHandler(SoftwareSaveEvent.TYPE, handler);
	}

	@Override
	public HandlerRegistration addSoftwareChangeHandler(SoftwareChangeHandler handler) {
	...
	private void selectPermissionForUser(String name) {
		...
		if (software.getPermission().ownernameExists(username) || PermUtils
				.getAccessLevelForUser(software.getPermission(), username, software.getId()).equals("Write")) {
			...
			UserREST.getUserRoles(username, new Callback<List<String>, Throwable>() {
				...
				public void onSuccess(List<String> roles) {
					if (roles != null && roles.contains("admin")) {
						...
						api.getPropertyAccessLevelForUser(software.getName(), propidselected, username,
	...
	private void submitPermissionForm() {
		...
		if (session != null) {
			...
			if (software.getPermission().ownernameExists(loggedinuser) || session.getRoles().contains("admin")) {
