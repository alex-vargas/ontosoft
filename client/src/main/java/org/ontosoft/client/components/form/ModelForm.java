...
import org.ontosoft.client.components.form.events.HasModelHandlers;
...
import org.ontosoft.client.components.form.events.ModelChangeHandler;
import org.ontosoft.client.components.form.events.ModelSaveEvent;
import org.ontosoft.client.components.form.events.ModelSaveHandler;
...
import org.ontosoft.client.components.form.formgroup.PropertyModelFormGroup;
...
public class ModelForm extends Composite implements HasModelHandlers, HasPluginHandlers {
	...
	Map<String, PropertyModelFormGroup> propeditors;

	interface Binder extends UiBinder<Widget, ModelForm> {
	};
	...
	public ModelForm() {
		...
		propeditors = new HashMap<String, PropertyModelFormGroup>();
	...
	@Override
	public HandlerRegistration addModelSaveHandler(ModelSaveHandler handler) {
		return handlerManager.addHandler(ModelSaveEvent.TYPE, handler);
	}
	...
	public void setRawPropertyValues(String propid, List<Object> values) {
		PropertyModelFormGroup ed = propeditors.get(propid);
		ed.setRawValue(values);
		model.addPropertyValues(propid, ed.getValue());
		fireEvent(new ModelChangeEvent(model));
	}
	...
	public void showCategoryItems(String categoryId) {
		...
		for (PropertyModelFormGroup editor : propeditors.values())
	...
	public void createFormItems() {
		...
		MetadataType modelType = vocabulary.getType(model.getType());
		List<MetadataProperty> modelProps = vocabulary.getPropertiesForType(modelType);
		modelProps = vocabulary.filterUneditableProperties(modelProps);
		modelProps = vocabulary.orderProperties(modelProps);

		for (final MetadataProperty mprop : modelProps) {
			PropertyModelFormGroup editor = new PropertyModelFormGroup(mprop, this);
	...
	public void layout() {
		for (PropertyModelFormGroup ed : propeditors.values())
			ed.layout();
	}
	...
	private void initAgents() {
		ArrayList<Authorization> authorizations = new ArrayList<Authorization>(
				this.model.getPermission().getAuthorizations().values());
		...
		for (Iterator<Authorization> iter = authorizations.listIterator(); iter.hasNext();) {
			...
			if (!permusers.contains(auth.getAgentName()) && auth.getAccessMode().getMode().equals("Write")
					&& (auth.getAccessToObjId().equals(propid) || auth.getAccessToObjId().equals(this.model.getId()))
	...
	private void setBrowsePermissionHeader() {
		...
		String propid = KBConstants.ONTNS() + this.propidselected;
		String mode = PermUtils.getAccessLevelForUser(this.model, "*", propid);
	...
	private void selectPermissionForUser(String name) {
		...
		if (model.getPermission().ownernameExists(username)
				|| PermUtils.getAccessLevelForUser(model.getPermission(), username, model.getId()).equals("Write")) {
			...
			UserREST.getUserRoles(username, new Callback<List<String>, Throwable>() {
				...
				public void onSuccess(List<String> roles) {
					if (roles != null && roles.contains("admin")) {
						...
						api.getPropertyAccessLevelForUser(model.getName(), propidselected, username,
	...
	private void submitPermissionForm() {
		...
		if (session != null) {
			...
			if (model.getPermission().ownernameExists(loggedinuser) || session.getRoles().contains("admin")) {
				...
				this.api.setPropertyPermissionForUser(model.getName(), authorization,
