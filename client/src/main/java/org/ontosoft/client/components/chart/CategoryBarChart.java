...
public class CategoryBarChart extends CategoryChartBase {
	...
	class BarDetails {
		...
		public BarDetails(Selection categoryBar, Selection backgroundBar, Selection fillBar, Selection fillOptBar) {
	...
	public void drawCategories(String categoryid) {
		...
		for (final String catid : topcat.getChildren()) {
			...
			final Selection bar = level.append("g").attr("id", catid);

			final Selection bgbar = bar.append("rect").attr("rx", cornerRadius).attr("ry", cornerRadius)
					.attr("x", startx).attr("y", caty).attr("width", mainBarWidth).attr("height", catbary - caty + cath)
					.attr("fill", bgcolor);

			if (this.isEventEnabled())
				bgbar.style("cursor", "pointer");

			final Selection donebar = bar.append("rect").attr("pointer-events", "none").attr("rx", cornerRadius)
					.attr("ry", cornerRadius).attr("x", startx).attr("y", catbary).attr("width", 0)
					.attr("stroke-width", 1).attr("stroke", bgcolor).attr("height", cath).attr("fill", "none");

			final Selection doneoptbar = bar.append("rect").attr("pointer-events", "none").attr("rx", cornerRadius)
					.attr("ry", cornerRadius).attr("x", optbarx).attr("y", catbary).attr("opacity", 0).attr("width", 0)
					.attr("stroke-width", 1).attr("stroke", "#f5f5f5").attr("height", cath).attr("fill", "done");

			this.categoryBars.put(catid, new BarDetails(bar, bgbar, donebar, doneoptbar));

			bar.append("text").attr("pointer-events", "none").style("font-size", 1.5 * fontsize + "px")
					.style("font-family", font).style("font-weight", "bold").style("fill", textcolor)
					.attr("class", "maintext").attr("x", startx + 4).attr("y", catlabely).text(label);

			bar.append("text").attr("pointer-events", "none").style("font-size", fontsize + "px")
					.style("font-family", font).style("fill", textcolor).attr("class", "maintext").attr("x", startx + 4)
					.attr("y", catsublabely).text(sublabel);
	...
	private void makeBarBigger(Selection bar, boolean animate) {
		if (animate) {
			bar.transition().duration(induration).attr("height", categoryHeight).attr("transform",
					"translate(0," + (categoryBarHeight - categoryHeight) + ")");
		} else {
			bar.attr("height", categoryHeight).attr("transform",
					"translate(0," + (categoryBarHeight - categoryHeight) + ")");
		}
	}
	...
	private void makeBarSmaller(Selection bar, boolean animate) {
		if (animate) {
			bar.transition().duration(outduration).attr("height", categoryBarHeight).attr("transform",
					"translate(0,0)");
		} else {
			bar.attr("height", categoryBarHeight).attr("transform", "translate(0,0)");
		}
	}
	...
	public void fillCategories(boolean animate, boolean quickanimation) {
		...
		for (String catId : this.categoryBars.keySet()) {
			...
			if (animate) {
				...
				bd.fillBar.transition().duration(duration).attr("width", mainBarWidth);

				bd.fillOptBar.attr("width", 0);
				bd.fillOptBar.transition().duration(duration).attr("width", optBarWidth);
			} else {
