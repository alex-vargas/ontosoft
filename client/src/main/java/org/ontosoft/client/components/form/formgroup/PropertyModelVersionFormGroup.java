...
public class PropertyModelVersionFormGroup extends FormGroup implements HasPluginHandlers {
	...
	public PropertyModelVersionFormGroup(final MetadataProperty property, ModelVersionForm formview) {
		super();
		...
		this.version = formview.getModelVersion();
	...
	private void updatePropertyPermissions() {
		...
		if (session != null) {
			 ...
			 */
			if (version.getPermission().ownernameExists(username) || session.getRoles().contains("admin")) {
	...
	private void addEntityEditor(final Entity entity, final HelpBlock infoblock) {
		...
		try {
			...
			if (entity instanceof ComplexEntity && property.isMultiple()
					&& entity.getType() == KBConstants.ONTNS() + "Function"
					&& entity.getType() != KBConstants.ONTNS() + "ModelVersion") {
			...
			ip.addEntityChangeHandler(new EntityChangeHandler() {
				...
				public void onEntityChange(EntityChangeEvent event) {
					...
					form.fireEvent(new ModelVersionChangeEvent(version));
	...
	private Entity getNewEntity(Object value) {
		...
		MetadataClass topclass = vocabulary.getType(KBConstants.ONTNS() + "ModelVersion");
		MetadataClass eclass = vocabulary.getType(proptype);
		if (vocabulary.isA(eclass, topclass) || property.getId() == KBConstants.ONTNS() + "affectsSoftwareFunction") {
	...
	private void runPlugin(final String pluginname) {
		SoftwareREST api = SoftwareREST.get(Config.getServerURL());
		api.runPlugin(pluginname, version, new Callback<PluginResponse, Throwable>() {
			@Override
