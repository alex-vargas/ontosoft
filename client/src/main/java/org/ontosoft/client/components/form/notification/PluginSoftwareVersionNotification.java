...
public class PluginSoftwareVersionNotification extends Panel {
	PluginResponse response;
	String target;
	String parentid;
	SoftwareVersionForm form;

	public PluginSoftwareVersionNotification(PluginResponse response, String parentid, SoftwareVersionForm form) {
	...
	private PanelHeader getPanelHeader() {
		final PluginSoftwareVersionNotification me = this;
	...
	private void addTableToBody(PanelBody body) {
		...
		final ListDataProvider<ResponseDetails> dataProvider = new ListDataProvider<ResponseDetails>();
		...
		final Column<ResponseDetails, String> okbutton = new Column<ResponseDetails, String>(
				new ButtonCell(IconType.CHECK, ButtonType.INFO, ButtonSize.SMALL)) {
			@Override
		...
		okbutton.setFieldUpdater(new FieldUpdater<ResponseDetails, String>() {
			...
			public void update(int index, ResponseDetails details, String value) {
				form.setRawPropertyValues(details.getProperty().getId(), details.getPropertyValues());
		...
		/*
		 * final Column<ResponseDetails, String> nobutton = new Column<ResponseDetails,
		 * String>( new ButtonCell(IconType.REMOVE, ButtonType.DANGER,
		 * ButtonSize.SMALL)) {
		 * 
		 * @Override public String getValue(ResponseDetails details) { return ""; } };
		 * nobutton.setCellStyleNames("button-cell"); nobutton.setFieldUpdater(new
		 * FieldUpdater<ResponseDetails, String>() {
		 * 
		 * @Override public void update(int index, ResponseDetails details, String
		 * value) { dataProvider.getList().remove(details);
		 * if(dataProvider.getList().size() == 0) me.removeFromParent(); } });
