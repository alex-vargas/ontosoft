...
public class ModelVersionListView extends ParameterizedViewImpl 
  implements ModelVersionListPresenter.MyView {
  ...
  CellTable<ModelVersionSummary> table = new CellTable<ModelVersionSummary>(40);
  ...
  private List<ModelVersionSummary> allModelsList;
  private HashMap<String, Boolean> filteredModelIdMap =
      new HashMap<String, Boolean>();
  private ListDataProvider<ModelVersionSummary> listProvider = 
      new ListDataProvider<ModelVersionSummary>();

  private List<ModelVersionSummary> selections;
  
  private Comparator<ModelVersionSummary> swcompare;
  
  interface Binder extends UiBinder<Widget, ModelVersionListView> {
  }
  ...
  public void initList() {
    ...
    for(final String sname : apis.keySet()) {
      ...
      allModelsList = new ArrayList<ModelVersionSummary>();
      sapi.getModelVersionList(modelname, new Callback<List<ModelVersionSummary>, Throwable>() {
        @Override
        public void onSuccess(List<ModelVersionSummary> list) {
          for(ModelVersionSummary sum : list) {
            sum.setExternalRepositoryId(sname);
            sum.setExternalRepositoryUrl(clientUrls.get(sname));
          }
          allModelsList.addAll(list);
          Collections.sort(allModelsList, swcompare);
          for(ModelVersionSummary sum : list)
          ...
          if(loaded.size() == apis.size()) {
            ...
            listProvider.getList().addAll(allModelsList);
            listProvider.flush();
            initMaterial();
  ...
  private void initTable() {
    ListHandler<ModelVersionSummary> sortHandler =
        new ListHandler<ModelVersionSummary>(listProvider.getList());
    
    selections = new ArrayList<ModelVersionSummary>();
    
    table.addColumnSortHandler(sortHandler);
    table.setEmptyTableWidget(new Label("No model found.."));
    
    this.swcompare = new Comparator<ModelVersionSummary>() {
      @Override
      public int compare(ModelVersionSummary sw1, ModelVersionSummary sw2) {
    ...
    Column<ModelVersionSummary, SafeHtml> namecol = new Column<ModelVersionSummary, SafeHtml>(progressCell) {
        @Override
        public SafeHtml getValue(ModelVersionSummary summary) {
    ...
    Column<ModelVersionSummary, String> deletecolumn = 
        new Column<ModelVersionSummary, String>(
            new ButtonCell(IconType.REMOVE, ButtonType.DANGER, ButtonSize.EXTRA_SMALL)) {
      @Override
      public String getValue(ModelVersionSummary details) {
        return "Delete";
      }
      @Override
      public String getCellStyleNames(Context context,
          ModelVersionSummary summary) {
        UserSession session = SessionStorage.getSession();
    ...
    deletecolumn.setFieldUpdater(new FieldUpdater<ModelVersionSummary, String>() {
      @Override
    ...
    // Edit Button Column
    final Column<ModelVersionSummary, String> editcol = 
        new Column<ModelVersionSummary, String>(
    ...
    editcol.setFieldUpdater(new FieldUpdater<ModelVersionSummary, String>() {
      @Override
      public void update(int index, ModelVersionSummary summary, String value) {
    ...
    Column<ModelVersionSummary, Boolean> checkboxcolumn = 
        new Column<ModelVersionSummary, Boolean>(new CheckboxCell(true, true)) {
        @Override
        public Boolean getValue(ModelVersionSummary summary) {
          return selections.contains(summary);
        }
    };
    checkboxcolumn.setFieldUpdater(new FieldUpdater<ModelVersionSummary, Boolean>() {
        @Override
        public void update(int index, ModelVersionSummary summary, Boolean value) {
          if(value) 
    ...
    // Set row styles
    table.setRowStyles(new RowStyles<ModelVersionSummary>() {
      @Override
      public String getStyleNames(ModelVersionSummary summary, int rowIndex) {
  ...
  void updateList() {
    ...
    for(ModelVersionSummary summary : allModelsList) {
  ...
  private void removeFromList(ModelVersionSummary summary) {
    filteredModelIdMap.remove(summary);
    allModelsList.remove(summary);
  }
    ...
    if(modellabel.validate(true)) {
      ...
      this.api.publishModel(tmpsw, new Callback<Model, Throwable>() {
        public void onSuccess(Model sw) {
          ...
          // Go to the new item
          History.newItem(NameTokens.publishModel + "/" + sw.getName());
  ...
  void onFacetSelection(FacetSelectionEvent event) {
    ...
    final List<ModelVersionSummary> facetList = new ArrayList<ModelVersionSummary>();
    for(final String sname : apis.keySet()) {
      SoftwareREST sapi = apis.get(sname);
      sapi.getModelVersionListFaceted(facets.getFacets(), modelname,
          new Callback<List<ModelVersionSummary>, Throwable>() {
        @Override
        public void onSuccess(List<ModelVersionSummary> list) {
          ...
          if(loaded.size() == apis.size()) {
            ...
            for(ModelVersionSummary flist: facetList)
  ...
  void onCompareButtonClick(ClickEvent event) {
    ...
    if(selections.size() < 2)
      Window.alert("Select atleast 2 models to compare.\n\n"
          + "Click on the checkbox next to the compare button.\n"
          + "Then select models using checkboxes in each row");
    else if(selections.size() > 10)
      Window.alert("Cannot compare more than 10 at a time");    
    else {
      int i=0;
      for(ModelVersionSummary summary : selections) {
        if(i > 0) idtext += ",";
        if(!summary.getExternalRepositoryId().equals(SoftwareREST.LOCAL))
