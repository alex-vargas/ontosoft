...
public class PropertyModelFormGroup extends FormGroup implements HasPluginHandlers {
	...
	public PropertyModelFormGroup(final MetadataProperty property, ModelForm modelView) {
		super();
		...
		this.vocabulary = modelView.getVocabulary();
	...
	private void initializeProperties() {
		...
		List<Entity> propEntities = model.getPropertyValues(property.getId());
	...
	private void updatePropertyPermissions() {
		...
		if (session != null) {
			...
			String swaccesslevel = PermUtils.getAccessLevelForUser(model, username, model.getId());
			/*
			 * Property permissions can only be updated by the admin or software owner.
			 */
			if (model.getPermission().ownernameExists(username) || session.getRoles().contains("admin")) {
				...
				String propaccesslevel = PermUtils.getAccessLevelForUser(model, username, property.getId());
	...
	public void setValue(List<Entity> entities) {
		...
		/*
		 * If user is admin or software owner, show the set permission button. Else if
		 * property is locked, show locked icon. Else if user is software editor, give
		 * complete edit access and add no controls. Else if user is only property
		 * editor, give edit access to the specific property and add no controls.
		 */
		...
		try {
			...
			ip.addEntityChangeHandler(new EntityChangeHandler() {
				...
				public void onEntityChange(EntityChangeEvent event) {
					...
					model.updatePropertyValue(property.getId(), e);
					form.fireEvent(new ModelChangeEvent(model));
				...
				public void onClick(ClickEvent event) {
					if (property.isMultiple() && entities.size() > 1) {
						...
						if (entity.getValue() != null) {
							...
							model.updatePropertyValue(property.getId(), entity);
							form.fireEvent(new ModelChangeEvent(model));
						}
						// GWT.log(software.getPropertyValues(property.getId()).toString());
					} else {
			...
			Provenance prov = this.model.getProvenance();
	...
	private Entity getNewEntity(Object value) {
		...
		String id = GUID.randomEntityId(model.getId(), proptype);

		// Convert software entities to enumerations
		MetadataClass topclass = vocabulary.getType(KBConstants.MODELCATALOGURINS() + "Model");
	...
	private void handleEntityPlugins(Entity e) {
		...
		for (Plugin plugin : property.getPlugins()) {
			if (!plugin.isAutomaticallyTriggered()) {
				...
				} else if (e.getValue() != null && e.getValue().toString().matches(plugin.getValueMatchRegex())) {
					match = true;
				}
				...
				// For Triggers
				if (e.getValue() != null && e.getValue().toString().matches(plugin.getValueMatchRegex())) {
	...
	private void runPlugin(final String pluginname) {
		SoftwareREST api = SoftwareREST.get(Config.getServerURL());
		api.runPlugin(pluginname, model, new Callback<PluginResponse, Throwable>() {
			@Override
