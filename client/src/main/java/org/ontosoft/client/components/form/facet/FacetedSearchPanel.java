...
public class FacetedSearchPanel extends PanelGroup implements HasFacetHandlers {
	...
	public void showFacetGroups(Vocabulary vocabulary) {
		...
		Map<String, List<MetadataProperty>> facetProperties = new TreeMap<String, List<MetadataProperty>>();
		for (MetadataProperty prop : swprops) {
			if (prop.getSearchConfig() != null && prop.getSearchConfig().getFacet() != null) {
		...
		for (String facetId : facetProperties.keySet()) {
			...
			if (type == null || type.getName().equals("Software") || (type.getName().equals("SoftwareVersion"))
					|| type.getName().equals("Model"))
				continue;

			if (vocabulary.isA(type, enumtype)) { // TODO: && type.getEnumerations().size() > 0) {
				FacetSelector facetselector = new FacetSelector(vocabulary, facetId, props, type, this.id);
	...
	@Override
	public HandlerRegistration addFacetSelectionHandler(FacetSelectionHandler handler) {
