...
public class ModelConfigurationListView extends ParameterizedViewImpl
	...
	private HashMap<String, Boolean> filteredModelIdMap = new HashMap<String, Boolean>();
	private ListDataProvider<ModelConfigurationSummary> listProvider = new ListDataProvider<ModelConfigurationSummary>();
...
//			}, false);
	...
	private void initTable() {
		ListHandler<ModelConfigurationSummary> sortHandler = new ListHandler<ModelConfigurationSummary>(
		...
		Column<ModelConfigurationSummary, SafeHtml> namecol = new Column<ModelConfigurationSummary, SafeHtml>(
				progressCell) {
			@Override
			public SafeHtml getValue(ModelConfigurationSummary summary) {
				SafeHtmlBuilder sb = new SafeHtmlBuilder();

				String link = "#" + NameTokens.modelConfigurations + "/" + summary.getSoftwareSummary().getName() + ":"
						+ summary.getName();
				...
				if (summary.getSoftwareSummary().getSoftwareName() != null)
					sb.appendHtmlConstant(
							"<a href='" + modelLink + "'>" + summary.getSoftwareSummary().getSoftwareName() + "</a>");
				else
					sb.appendHtmlConstant(
							"<a href='" + modelLink + "'>" + summary.getSoftwareSummary().getLabel() + "</a>");
				sb.appendHtmlConstant(" >> ");
		...
		Column<ModelConfigurationSummary, String> deletecolumn = new Column<ModelConfigurationSummary, String>(
			...
			@Override
			public String getCellStyleNames(Context context, ModelConfigurationSummary summary) {
				UserSession session = SessionStorage.getSession();
		...
		// Edit Button Column
		final Column<ModelConfigurationSummary, String> editcol = new Column<ModelConfigurationSummary, String>(
		...
		Column<ModelConfigurationSummary, Boolean> checkboxcolumn = new Column<ModelConfigurationSummary, Boolean>(
		...
		for (ModelConfigurationSummary summary : allModelList) {
			if (filteredModelIdMap.containsKey(summary.getId())) {
				if ((value == null || value.equals("")
						...
						&& (modelName == null || (modelName != null && modelName != ""
								&& summary.getSoftwareSummary().getName() == modelName)))
					listProvider.getList().add(summary);
			}
		}
		this.listProvider.flush();
