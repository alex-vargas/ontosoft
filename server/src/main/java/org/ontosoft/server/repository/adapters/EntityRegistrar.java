...
public class EntityRegistrar {
	private static HashMap<String, Class<IEntityAdapter>> adapters = new HashMap<String, Class<IEntityAdapter>>();
	...
	public static IEntityAdapter getAdapter(KBAPI kb, KBAPI ontkb, KBAPI enumkb, String classid) {
		...
		try {
			return adapterClass.getConstructor(KBAPI.class, KBAPI.class, String.class).newInstance(kb, ontkb, classid);
		} catch (Exception e1) {
			try {
				return adapterClass.getConstructor(KBAPI.class, KBAPI.class, KBAPI.class, String.class).newInstance(kb,
						ontkb, enumkb, classid);
			} catch (Exception e2) {
				e1.printStackTrace();
				e2.printStackTrace();
			}
