...
public class PropertyFormGroup extends FormGroup implements HasPluginHandlers {
	...
	private void updatePropertyPermissions() {
		...
		if (session != null) {
			 ...
			 */
			if (software.getPermission().ownernameExists(username) || session.getRoles().contains("admin")) {
	...
	private void runPlugin(final String pluginname) {
		SoftwareREST api = SoftwareREST.get(Config.getServerURL());
		api.runPlugin(pluginname, software, new Callback<PluginResponse, Throwable>() {
			@Override
