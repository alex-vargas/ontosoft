...
public class FacetSelector extends Panel implements HasFacetHandlers {
	...
	public FacetSelector(Vocabulary vocabulary, String facetId, List<MetadataProperty> props, MetadataType type,
			String parentid) {
	...
	public void fetchEnumerations() {
		...
		for (final String sname : this.apis.keySet()) {
			...
			sapi.getEnumerationsForType(type.getId(), new Callback<List<MetadataEnumeration>, Throwable>() {
				@Override
				public void onSuccess(List<MetadataEnumeration> enumlist) {
					...
					if (loaded.size() == apis.keySet().size()) {
						...
						Collections.sort(enumerations, new Comparator<MetadataEnumeration>() {
							@Override
							public int compare(MetadataEnumeration enum1, MetadataEnumeration enum2) {
