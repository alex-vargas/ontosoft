...
public class FunctionListView extends ParameterizedViewImpl implements FunctionListPresenter.MyView {
	...
	private ListDataProvider<FunctionSummary> listProvider = new ListDataProvider<FunctionSummary>();
	...
	private void initTable() {
		ListHandler<FunctionSummary> sortHandler = new ListHandler<FunctionSummary>(listProvider.getList());
		...
		Column<FunctionSummary, SafeHtml> namecol = new Column<FunctionSummary, SafeHtml>(progressCell) {
			...
			public SafeHtml getValue(FunctionSummary summary) {
				...
				String link = "#" + NameTokens.version + "/" + summary.getSoftwareSummary().getName() + ":"
						+ summary.getSoftwareVersionSummary().getName();
				String softwareLink = "#" + NameTokens.browse + "/" + summary.getSoftwareSummary().getName();
				String softwareVersionLink = "#" + NameTokens.version + "/" + summary.getSoftwareSummary().getName()
						+ ":" + summary.getSoftwareVersionSummary().getName();
				...
				if (summary.getSoftwareVersionSummary().getSoftwareName() != null)
					sb.appendHtmlConstant("<a href='" + softwareVersionLink + "'>"
							+ summary.getSoftwareVersionSummary().getSoftwareName() + "</a>");
				else
					sb.appendHtmlConstant("<a href='" + softwareVersionLink + "'>"
							+ summary.getSoftwareVersionSummary().getLabel() + "</a>");
		...
		// Edit Button Column
		final Column<FunctionSummary, String> editcol = new Column<FunctionSummary, String>(
			...
			@Override
			public String getCellStyleNames(Context context, FunctionSummary summary) {
				UserSession session = SessionStorage.getSession();
		...
		Column<FunctionSummary, Boolean> checkboxcolumn = new Column<FunctionSummary, Boolean>(
	...
	void onFacetSelection(FacetSelectionEvent event) {
		...
		for (final String sname : apis.keySet()) {
			SoftwareREST sapi = apis.get(sname);
			sapi.getFunctionListFaceted(facets.getFacets(), new Callback<List<FunctionSummary>, Throwable>() {
				@Override
				public void onSuccess(List<FunctionSummary> list) {
	...
	void onCompareButtonClick(ClickEvent event) {
		...
		if (selections.size() < 2)
			...
			for (FunctionSummary summary : selections) {
				...
				idtext += summary.getSoftwareSummary().getName() + ":" + summary.getSoftwareVersionSummary().getName()
						+ ":" + summary.getName();
				i++;
			}
