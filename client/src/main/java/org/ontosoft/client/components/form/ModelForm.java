...
implements HasModelHandlers, HasPluginHandlers {
	  ...
	  @Override
	  public HandlerRegistration addModelSaveHandler(
	      ModelSaveHandler handler) {
	  ...
	  private void initAgents() {
	    ...
	    for (Iterator<Authorization> iter = authorizations.listIterator(); iter.hasNext(); ) {
	      ...
	      if (!permusers.contains(auth.getAgentName()) && 
	        ...
	        (auth.getAccessToObjId().equals(propid) || 
	  ...
	  private void selectPermissionForUser(String name) {	 
		...
		if (model.getPermission().ownernameExists(username) ||
	      PermUtils.getAccessLevelForUser(model.getPermission(),
	    		  username, model.getId()).equals("Write")) {
	      ...
	      UserREST.getUserRoles(username, new Callback<List<String>, Throwable>() {
	        ...
	        public void onSuccess(List<String> roles) {
	          if (roles != null && roles.contains("admin")) {
	            ...
	            api.getPropertyAccessLevelForUser(model.getName(), propidselected, 
	  ...
	  private void submitPermissionForm() {
	    ...
	    if (session != null) {
	      ...
	      if (model.getPermission().ownernameExists(loggedinuser) ||
