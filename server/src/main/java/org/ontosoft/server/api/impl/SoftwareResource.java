...
public class SoftwareResource implements SoftwareService {
	...
	public List<SoftwareVersionSummary> listModelVersions(String software) {
		try {
			return this.repo.getAllModelVersions(software);
		} catch (Exception e) {
			e.printStackTrace();
			throw new RuntimeException("Exception: " + e.getMessage());
		}
	...
	public List<SoftwareVersionSummary> listSoftwareVersionWithFacets(
			...
			@PathParam("version") String software,
			@PathParam("isModel") boolean isModel) {
		try {
			if(isModel)
				return this.repo.getAllModelVersionsWithFacets(facets, software);
			else
	...
	public List<ModelConfigurationSummary> listModelConfigurationWithFacets(
			@JsonProperty("facets") List<EnumerationFacet> facets, @PathParam("version") String model) {
		try {
	...
	public SoftwareVersion getVersion(@PathParam("name") String name,
		...
		return getSoftwareVersion(false, name, version);
	}
	
	private SoftwareVersion getSoftwareVersion(boolean isModel, String name,
			String version) {
		...
		try {
			...
			return this.repo.getSoftwareVersion(isModel, swid, vid);
		} catch (Exception e) {
			e.printStackTrace();
			throw new RuntimeException("Exception: " + e.getMessage());
		}
	...
	public SoftwareVersion getModelVersion(@PathParam("name") String name,
			@PathParam("version") String version) {
		return getSoftwareVersion(true, name, version);
	}
	...
	@Produces("application/json")
	@Override
	public SoftwareFunction getSoftwareFunction(@PathParam("name") String name, @PathParam("version") String version,
	...
	private Software publishSoftware(Software software, boolean isModel) {
		try {
			String swid = "";
			if (isModel)
				swid = this.repo.addModel(software, (User) securityContext.getUserPrincipal());
			else
				swid = this.repo.addSoftware(software, (User) securityContext.getUserPrincipal(), false);
			if (swid != null) {
				software.setId(swid);
				return this.repo.getSoftware(swid);
	...
	private SoftwareVersion publishSoftwareVersion(boolean isModel, String name, SoftwareVersion version) {
		try {
			...
			if (vid != null) {
				version.setId(vid);
				return this.repo.getSoftwareVersion(isModel, name, vid);
				// response.sendRedirect(swid);
				// return software;
			}
			return null;
	...
	@RolesAllowed("user")
	@Override
	public Software update(@PathParam("name") String name, @JsonProperty("software") Software software) {
		try {
			String swid = name;
			if (!name.startsWith("http:"))
				swid = repo.LIBNS() + name;
			if (!this.repo.updateSoftware(software, swid, (User) securityContext.getUserPrincipal()))
	...
	@RolesAllowed("user")
	@Override
	public void deleteModelVersion(@PathParam("name") String name, @PathParam("vname") String vname) {
		try {
			String swid = name;
			if (!name.startsWith("http:"))
				swid = repo.LIBNS() + name;
	...
	@RolesAllowed("user")
	@Override
	public PluginResponse runPlugin(@PathParam("name") String name, @JsonProperty("software") Software software) {
	...
	public AccessMode getPropertyAccessLevelForUser(@PathParam("name") String swname,
			@PathParam("propname") String propname, @PathParam("username") String username) {
		String swid = swname;
		if (!swname.startsWith("http:"))
			swid = repo.LIBNS() + swname;
	...
	public Boolean setPropertyPermissionForUser(@PathParam("name") String name,
		...
		return this.repo.setPropertyPermissionForUser((User) securityContext.getUserPrincipal(), swid, authorization);
	}
	...
	@Produces("application/json")
	@Consumes("application/json")
	public Boolean addSoftwareOwner(@PathParam("name") String swname, @PathParam("username") String username) {
		String swid = swname;
		if (!swname.startsWith("http:"))
			swid = repo.LIBNS() + swname;

		return this.repo.addSoftwareOwner((User) securityContext.getUserPrincipal(), swid, username);
	}
	...
	@Produces("application/json")
	@Consumes("application/json")
	public Boolean removeSoftwareOwner(@PathParam("name") String swname, @PathParam("username") String username) {
		String swid = swname;
		if (!swname.startsWith("http:"))
			swid = repo.LIBNS() + swname;

		return this.repo.removeSoftwareOwner((User) securityContext.getUserPrincipal(), swid, username);
	}
