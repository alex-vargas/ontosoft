...
public class EntityGenerator extends Generator {
	...
	/**
	 * Generate Java code for factory class capable of returning instance for a
	 * class name.
	 * <p>
	 * It is not true introspection. In fact the whole factory class is a mapper;
	 * mapping class name to constructor call. Like
	 * {@code if (className=="my.code.Foo1") return new my.code.Foo1();} Only small
	 * section of classes can be instantiable this way. Supported classes are
	 * filtered based marker interface, annotation or regex. This implementation is
	 * uses marker interface {@link EntryPoint}.
	 * 
	 * @param logger
	 * @param context
	 *            provides list of all visible classes, see
	 *            {@link GeneratorContext#getTypeOracle()} , and stream for printing
	 *            the Java code, see {@link SourceWriter}.
	 * @param typeName
	...
	public String generate(TreeLogger logger, GeneratorContext context, String typeName)
		...
		// prepare Composer. Composer prepares shell of the Java code, so we need to set
		// package and class name
		ClassSourceFileComposerFactory composer = new ClassSourceFileComposerFactory(genPackageName, genClassName);

		// register (add) required imports to Composer
		// but the generated class uses fully qualified names everywhere, no imports
		// required
		...
		if (printWriter != null) {
			...
			// generate class content, content of the Java file, contructor and factory
			// method
	...
	/**
	 * Filter all classes visible to GWT client side and return only those deemed
	 * instantiable through the factory class we are generating (that is
	 * EntityInputFactoryImpl).
	 * <p>
	 * Allowing all classes to be instantiable would potentially generate too huge
	 * class method perhaps even too huge to compile. There must be some
	 * sub-selection. Classes can be filtered based on marker interface, or class
	 * annotation see {@link JClassType#getAnnotation(Class)}, or some regex
	 * matching based on package or class name.
	 * 
	 * @param context
	 * @return Filter all classes visible to GWT client side and return only those
	 *         deemed instantiable
	 */
	...
	private void generateFactoryClass(String genClassName, ClassSourceFileComposerFactory composer,
		...
		// flush all writes to stream; mark generating process as finished and ready for
		// compiler
	 ...
	 * @param sourceWriter
	 */
	private void generateFactoryMethods(List<JClassType> allInstantiableClasses, SourceWriter sourceWriter,
			TreeLogger logger) {
		// instantiate method
