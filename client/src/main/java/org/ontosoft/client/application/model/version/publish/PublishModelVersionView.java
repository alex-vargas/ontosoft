...
public class PublishModelVersionView extends ParameterizedViewImpl implements PublishModelVersionPresenter.MyView {
	...
	private void initAgents() {
		...
		for (Iterator<Authorization> iter = authorizations.listIterator(); iter.hasNext();) {
			...
			if (!permusers.contains(auth.getAgentName()) && auth.getAccessMode().getMode().equals("Write")
					&& auth.getAccessToObjId().equals(this.version.getId()) && !auth.getAgentName().equals("*")) {
				authlist.add(auth);
	...
	private void initModel(final String versionname, final boolean reload) {
		...
		// permissiondialog.setTitle("Set Permissions for " + swlabel.substring(0,
		// 1).toUpperCase() + swlabel.substring(1));
		// }
		// @Override
		// public void onFailure(Throwable reason) {
		// reloadbutton.setIconSpin(false);
	...
	void onPieSelect(CategorySelectionEvent event) {
		...
		if (pcat != null) {
			History.replaceItem(
					NameTokens.publishModelVersion + "/" + softwarename + ":" + versionname + "/" + pcat.getName(),
					false);
			barchart.setActiveCategoryId(null, false);
		...
		UserREST.getUserRoles(username, new Callback<List<String>, Throwable>() {
			...
			public void onSuccess(List<String> roles) {
				if (roles != null && roles.contains("admin")) {
					...
					api.getSoftwareAccessLevelForUser(version.getName(), username,
	...
	private void submitPermissionForm() {
		...
		if ((session != null && session.getRoles().contains("admin"))
			...
			if (ownerrole.getValue() == true) {
				this.api.addSoftwareOwner(version.getName(), username, new Callback<Boolean, Throwable>() {
					@Override
					public void onFailure(Throwable reason) {
