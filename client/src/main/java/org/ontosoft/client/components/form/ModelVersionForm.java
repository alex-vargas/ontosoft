...
import org.ontosoft.client.components.form.events.HasModelVersionHandlers;
...
import org.ontosoft.client.components.form.events.ModelVersionChangeHandler;
import org.ontosoft.client.components.form.events.ModelVersionSaveEvent;
import org.ontosoft.client.components.form.events.ModelVersionSaveHandler;
...
import org.ontosoft.client.components.form.formgroup.PropertyModelVersionFormGroup;
...
public class ModelVersionForm extends Composite implements HasModelVersionHandlers, HasPluginHandlers {
	...
	Map<String, PropertyModelVersionFormGroup> propeditors;

	interface Binder extends UiBinder<Widget, ModelVersionForm> {
	};
	...
	public ModelVersionForm() {
		initWidget(uiBinder.createAndBindUi(this));
		propeditors = new HashMap<String, PropertyModelVersionFormGroup>();
	...
	@Override
	public HandlerRegistration addModelVersionSaveHandler(ModelVersionSaveHandler handler) {
		return handlerManager.addHandler(ModelVersionSaveEvent.TYPE, handler);
	}

	@Override
	public HandlerRegistration addModelVersionChangeHandler(ModelVersionChangeHandler handler) {
		return handlerManager.addHandler(ModelVersionChangeEvent.TYPE, handler);
	}
	...
	public void setRawPropertyValues(String propid, List<Object> values) {
		PropertyModelVersionFormGroup ed = propeditors.get(propid);
		ed.setRawValue(values);
		version.addPropertyValues(propid, ed.getValue());
		fireEvent(new ModelVersionChangeEvent(version));
	}
	...
	public void showCategoryItems(String categoryId) {
		...
		for (PropertyModelVersionFormGroup editor : propeditors.values())
	...
	public void createFormItems() {
		...
		for (final MetadataProperty mprop : swprops) {
			PropertyModelVersionFormGroup editor = new PropertyModelVersionFormGroup(mprop, this);
	...
	public void layout() {
		for (PropertyModelVersionFormGroup ed : propeditors.values())
			ed.layout();
	}
