...
public class ModelBrowseView extends ParameterizedViewImpl 
	implements ModelBrowsePresenter.MyView {

	interface Binder extends UiBinder<Widget, ModelBrowseView> {}
...
//	Software software;
	...
	public interface SoftwareCodec extends JsonEncoderDecoder<Model> {}
	SoftwareCodec codec;
	...
	// If some parameters are passed in, initialize the model and interface
	...
	private void initModel(String modelName) {
	    ...
	    this.api.getModel(modelName, new Callback<Model, Throwable>() {
	      @Override
	...
	private void initModelRDF() {
	  this.api.getModelRDF(model.getId(), new Callback<String, Throwable>() {
	    @Override
	    ...
	    @Override
	    public void onFailure(Throwable exception) {
	      GWT.log("Error fetching Model RDF", exception);
	    }
	  });
	}
	...
	public void showModel(Model model) {
	  ...
	  Entity modelName = model.getPropertyValue(KBConstants.ONTNS()+"hasModelName");
	  if (modelName != null)
	  	modelTitle.setText(modelName.getValue().toString());
	  else
		  modelTitle.setText(model.getLabel());

	  List<Entity> authorlist = model.getPropertyValues(KBConstants.ONTNS()+"hasModelCreator");
	  String authors = authorlist.toString();
	  if(authorlist.isEmpty())
	    authors = "[No author listed]";
	  modelTitle.setSubText(authors);
	  modelBody.clear();
	  ...
	  for(String lvl1catid: topcat.getChildren()) {
	    ...
	    for(String lvl2catid: lvl1cat.getChildren()) {
	      ...
	      List<MetadataProperty> props = vocabulary.getPropertiesForType(modelType);
	      props.retainAll(vocabulary.getPropertiesInCategory(lvl2cat));
	      props = vocabulary.orderProperties(props);
	      
	      String html = browser.getEntitiesHTML(model, props, false);
          HTML lvl2html = new HTML(html);
          if(hasSomePropertyValues(props, model)) {
            hasSomeValues = true;
          }
          else {
	...
	private boolean hasSomePropertyValues(List<MetadataProperty> props, Model model) {
	  for(MetadataProperty prop : props) {
	    if(model.getPropertyValues(prop.getId()).size() > 0) {
	      return true;
	    }
	  }
	  return false;
	...
	void onJsonButtonClick(ClickEvent event) {
	  openWindow("application/json", codec.encode(model).toString());
	}
