...
public class CategoryChartBase extends SimplePanel implements HasCategoryChartHandlers {
	...
	public boolean drawnCategories() {
		return this.drawncategories;
	}
	...
	protected void fadeOut(Selection item, boolean animate) {
		if (animate) {
			item.transition().duration(outduration).style("fill-opacity", 0.3);
		} else {
	...
	protected void fadeIn(Selection item, boolean animate) {
		if (animate) {
			item.transition().duration(induration / 2).style("fill-opacity", 1);
		} else {
	...
	protected Selection setGradient(Selection grad, double percentage) {
		double gradoffset = percentage < 80 ? 15 : (percentage < 95 ? 10 : (percentage < 100 ? 5 : 0));
		if (percentage > 0) {
			grad.append("stop").attr("offset", (percentage - gradoffset) + "%").style("stop-color", donecolor);
			grad.append("stop").attr("offset", percentage + "%").style("stop-color", midcolor);
		}
		grad.append("stop").attr("offset", (percentage + gradoffset) + "%").style("stop-color", notdonecolor);
		return grad;
	}
	...
	protected Selection setOptGradient(Selection grad, double percentage, double start) {
		...
		if (percentage > 0) {
			grad.append("stop").attr("offset", start + diff * (percentage - gradoffset) + "%").style("stop-color",
					donecolor);
			grad.append("stop").attr("offset", start + diff * percentage + "%").style("stop-color", midoptcolor);
		}
		grad.append("stop").attr("offset", start + diff * (percentage + gradoffset) + "%").style("stop-color",
				optcolor);
		return grad;
	...
	protected String addGreyscaleFilter(Selection defs) {
		...
		greyscale.append("feColorMatrix").attr("type", "matrix").attr("values",
